...
def admin_auth(f):
    ...
    def decorated_function(*args, **kwargs):
        admin = Admin.query.join(
            Role).filter(Role.id == Admin.role_id,
                         Admin.id == session['admin_id']
                         ).first()
        auths = admin.role.auths
        auths = list(map(lambda v: int(v), auths.split(',')))
        auths_list = Auth.query.all()
        urls = [i.url for i in  auths_list for v in auths if v == i.id]
...
def tag_edit(id=None):
    ...
    if form.validate_on_submit():
        ...
        if tag_count == 1 and tag.name != data['name']:  # 提交的标签==1,并且要修改的标签名称和自己现在写的标签名不一样
            flash('名称已存在', 'err')
...
def movie_list(page=None):
    if page is None:
        page = 1
    pagedata = Movie.query.join(Tag).filter(Tag.id == Movie.tag_id).order_by(
        Movie.addtime.desc()
    ).paginate(page=page, per_page=10)
    return render_template("admin/movie_list.html", pagedata=pagedata)
...
def preview_list(page=None):
    if page is None:
        page = 1
    pagedata = Preview.query.order_by(
...
def oplog_list(page=None):
    ...
    return render_template('admin/oplog_list.html', pagedata=pagedata)
...
def adminloginlog_list(page=None):
    ...
    return render_template('admin/adminloginlog_list.html', pagedata=pagedata)
...
def userloginlog_list(page=None):
    ...
    return render_template('admin/userloginlog_list.html', pagedata=pagedata)
...
def role_edit(id=None):
    ...
    if request.method == 'GET':
        form.auths.data = [int(i) for i in role.auths.split(',')]
